...
class JobSupervisor:
    ...
    def _api_routing(self) -> None:
        ...
        async def start_job(job_name: str, job_config: dict):
            ...
            valid_job_names = ["audioprospector"]
            if job_name not in valid_job_names:
                h_log.create_log(3, "job_supervisor.JobSupervisor._api_routing", f"ENOK: API Call no. {self._api_hit_counter} - {log_api_endpoint} - Invalid job name")
                self._api_hit_counter+=1
                return JSONResponse(content={"status":"failed","reason":"Invalid job name"}, status_code=400)
            ''' Run job '''
            job_start_status, job_start_content = self._job_start(job_name, job_config)
            if not job_start_status:
                h_log.create_log(2, "job_supervisor.JobSupervisor._api_routing", f"ENOK: API Call no. {self._api_hit_counter} - {log_api_endpoint} - {str(job_start_content)}")
                self._api_hit_counter+=1
                return JSONResponse(content={"status":"failed","reason":str(job_start_content)}, status_code=400)
            h_log.create_log(4, "job_supervisor.JobSupervisor._api_routing", f"EOK: API Call no. {self._api_hit_counter} - {log_api_endpoint}")
            self._api_hit_counter+=1
            return JSONResponse(content={"status":"success","content":job_start_content}, status_code=201)
        
        @self._api.post("/api/job/stop",tags=["Job operations"])
        async def stop_job(job_name: str):
            log_api_endpoint = f"/api/job/start?job_name={job_name}"
            h_log.create_log(4, "job_supervisor.JobSupervisor._api_routing", f"S: API Call no. {self._api_hit_counter} - {log_api_endpoint}")
            ''' Check if job_name is valid '''
            valid_job_names = ["audioprospector"]
            if job_name not in valid_job_names:
                ...
                return JSONResponse(content={"status":"failed","reason":"Invalid job name"}, status_code=400)
            ''' Stop job '''
            job_stop_status, job_stop_content = self._job_stop(job_name)
            if not job_stop_status:
                h_log.create_log(2, "job_supervisor.JobSupervisor._api_routing", f"ENOK: API Call no. {self._api_hit_counter} - {log_api_endpoint} - {str(job_stop_content)}")
                self._api_hit_counter+=1
                return JSONResponse(content={"status":"failed","reason":str(job_stop_content)}, status_code=400)
            h_log.create_log(4, "job_supervisor.JobSupervisor._api_routing", f"EOK: API Call no. {self._api_hit_counter} - {log_api_endpoint}")
            self._api_hit_counter+=1
            return JSONResponse(content={"status":"success","content":job_stop_content}, status_code=201)
    ...
    def _job_start(self, job_name: str, job_config: dict) -> tuple:
        match job_name:
