...
from audio_prospector import AudioProspector as AudioProspector
import json
...
if __name__ == "__main__":
    h_log.create_log(4, "run.__main__", "Attempting to start service job-audio_prospector")
    j_ap = AudioProspector()
    '''
    ...
    try:
        ...
        h_log.create_log(2, "run.__main__", f"Failed to read configuration file. Reason: {str(e)}")
        h_log.create_log(1, "run.__main__", "Failed to start service job-audio_prospector")
        time.sleep(5), quit()
    ...
    '''
    Pass configuration to handlers & supervisor section
    '''
    ...
    if not db_init_status:
        ...
        h_log.create_log(1, "run.__main__", f"Failed to start service job-audio_prospector")
        time.sleep(5), quit()
    h_log.create_log(4, "run.__main__", f"Successfully initialized DB connection")
    '''
    '''
    h_log.create_log(4, "run.__main__", f"Successfully started service job-audio_prospector")
    j_ap.api_run()
