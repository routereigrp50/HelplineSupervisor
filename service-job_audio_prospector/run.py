...
if __name__ == "__main__":
    h_log.create_log(4, "run.__main__", "Attempting to start service job_audio_prospector")
    ...
    try:
        ...
        h_log.create_log(1, "run.__main__", "Failed to start service job_audio_prospector")
        time.sleep(5), quit()
    ...
    config_schema = {
        ...
        "properties": {
            ...
            "required": ["audio_file_path", "audio_file_extension", "scanning_interval"]
            },
            "database": {
            "type": "object",
            "properties": {
        ...
        "required": ["api", "job", "database", "logging"]
        }
    try:
    ...
    if not db_init_status:
        ...
        h_log.create_log(1, "run.__main__", f"Failed to start service job_audio_prospector")
        time.sleep(5), quit()
    h_log.create_log(4, "run.__main__", f"Successfully initialized DB connection")

    h_log.create_log(4, "run.__main__", f"Successfully started service job_audio_prospector")
