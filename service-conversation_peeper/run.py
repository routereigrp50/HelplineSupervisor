...
from conversation_peeper import ConversationPeeper
import json
import time
...
if __name__ == "__main__":
    h_log.create_log(4, "run.__main__", "Attempting to start service conversation_peeper")
    '''
    ...
    try:
        ...
        h_log.create_log(2, "run.__main__", f"Failed to read configuration file. Reason: {str(e)}")
        h_log.create_log(1, "run.__main__", "Failed to start service conversation_peeper")
        time.sleep(5), quit()
    ...
    try:
        try: h_log.set_debug(configuration["logging"]['local']['conversation_peeper']["debugging"]) 
        except KeyError: h_log.set_debug(configuration['logging']['global']['debugging']) 

        try: h_log.set_stdout_console(configuration["logging"]['local']['conversation_peeper']["stdout"]['console']['state']) 
        except KeyError: h_log.set_stdout_console(configuration['logging']['global']['stdout']['console']['state'])

        try: h_log.set_stdout_db(configuration["logging"]['local']['conversation_peeper']["stdout"]['database']['state'],configuration['database']['mongo_db_uri']) 
        ...
        try: h_log.set_stdout_file_path(configuration["logging"]['local']['conversation_peeper']["stdout"]['file']['file_path']) 
        except: h_log.set_stdout_file_path(configuration['logging']['global']['stdout']['file']['file_path'])

        try: h_log.set_stdout_file(configuration["logging"]['local']['conversation_peeper']["stdout"]['file']['state']) 
        ...
        h_log.create_log(1, "run.__main__", f"Failed to start service conversation_peeper")
        time.sleep(5), quit()
    h_log.create_log(4, "run.__main__", f"Successfully passed configuration to handlers")
    '''
    SECTION: CREATE AND RUN JOB
    '''
    h_log.create_log(4, "run.__main__", f"Successfully started service conversation_peeper")
    cp = ConversationPeeper(configuration['api']['conversation_peeper'])